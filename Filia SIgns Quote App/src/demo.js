...
Demo = {
	init: function () {
		Demo.el = {
			...
			blocks: $('#blocks'),
			canvas: $('#canvas')[0],
			size: $('#canvasSizeCalc'), // THIS IS WHERE I CAN CHANGE WHERE THE SIZE COMES FROM *********
			sort: $('#sort'),
			color: $('#color'),
			ratio: $('#ratio'),
			nofit: $('#nofit'),
		...
		Demo.el.size.change(Demo.run); // CHANGE THIS TO UPDATE BOARD WHEN SIZE IS MULTIPLIED *********
...
sizeOptionList.addEventListener('change', () => {
	$(Demo.run);
});
blockFlip.forEach((click) => {
